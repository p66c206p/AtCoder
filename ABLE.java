...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        long[] modpow = new long[252521];
        modpow[0] = 1;
        for (int i = 0; i < 252521-1; i++) modpow[i+1] = (modpow[i] * 10)%MOD;
        long[] modpow2 = new long[252521];
        modpow2[0] = 1;
        for (int i = 0; i < 252521-1; i++) modpow2[i+1] = (modpow2[0] + (modpow2[i] * 10)%MOD)%MOD;
        
        // Range Sum Query (区間更新の方)
        LazySegTree<F, Long> seg = new LazySegTree<>(n, (v1, v2) -> new F(((v1.val*modpow[v2.size])%MOD+v2.val)%MOD, v1.size+v2.size), (m1, m2) -> m2, (dat, lazy) -> new F((lazy*modpow2[dat.size-1])%MOD, dat.size), new F(0,0), 0L);
        ...
        while (q-- > 0) {
            ...
            if (type == 0) {
                ...
                int l = sc.nextInt()-1;
                int r = sc.nextInt();
                long res = seg.query(l, r).val;
            }
            ans.add(seg.query(0, n).val);
        }
