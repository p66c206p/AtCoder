...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Range Sum Query
        LazySegTree<F, P> seg = new LazySegTree<>(n, (v1, v2) -> new F((v1.val+v2.val)%MOD, (v1.size+v2.size)%MOD), (m1, m2) -> new P((m2.b*m1.b)%MOD, ((m2.b*m1.c)%MOD+m2.c)%MOD), (dat, lazy) -> new F(((dat.val*lazy.b)%MOD + (dat.size*lazy.c)%MOD)%MOD, dat.size), new F(0, 0), new P(1, 0));
        
        // 初期の配列を取得
        for (int i = 0; i < n; i++) {
            seg.set(i, new F(sc.nextLong(), 1));
        }
        seg.build();
